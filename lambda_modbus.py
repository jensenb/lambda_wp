...
class ConnectionManager:
    ...
    async def _init(self):
        ...
        self.influxdb_client = InfluxDBClientAsync(url=f"http://{self.influxdb_host}:{self.influxdb_port}",
    ...
    async def read_lambda_vals(self,
        register: int,
        ...
        cache_timeout: float=300.0) -> NoReturn:
...
async def create_connection_manager(lambda_host:str,
                                    lambda_port:int,
                                    influxdb_host: str,
                                    influxdb_port: int,
                                    influxdb_token: str,
                                    influxdb_org: str,
                                    influxdb_bucket: str) -> ConnectionManager:

    conn_man = ConnectionManager(lambda_host=lambda_host,
